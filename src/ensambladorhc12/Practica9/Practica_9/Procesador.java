...
public final class Procesador {
    ...
    static String agregarCeros(Long CONT_LOC) {
        return ("0000" + Long.toHexString(CONT_LOC).toUpperCase()).substring(Long.toHexString(CONT_LOC).toUpperCase().length());
    }
    
    static String agregarCeros4(String CONT_LOC) {
        return ("0000" + CONT_LOC.toUpperCase()).substring(CONT_LOC.toUpperCase().length());
    }
    static String agregarCeros8(String CONT_LOC) {
        return ("000000" + CONT_LOC.toUpperCase()).substring(CONT_LOC.toUpperCase().length());
    }
    
    public static String convertirRRaBinario(String tabla){
        if(tabla.toUpperCase().equals("X"))
            return "00";
        else if(tabla.toUpperCase().equals("Y"))
            return  "01";
        else if(tabla.toUpperCase().equals("SP"))
            return  "10";
        else if(tabla.toUpperCase().equals("PC"))
            return  "11";
        return null;
    }
}
