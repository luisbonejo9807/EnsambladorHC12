...
public final class EnsambladorHC12 {
    ...
    public EnsambladorHC12(String FILE_NAME) 
    {
        try{
    ...
    public static void main(String[] args) {
        ...
        if(!Files.exists(Paths.get(FILE_NAME)))
            EnsambladorHC12.writeError(0, "\n\tERROR: El archivo no existe");
        else
        {
            EnsambladorHC12 ensamblador = new EnsambladorHC12(FILE_NAME);
                ...
                {   
                    ...
                    if(i == lineas.length-1 )
                    {
                        if( palabra[0].matches("/^END$/i"))
                        {
                            ...
                            finDeArchivo = EnsambladorHC12.writeError(i+1, "\nERROR: El archivo no termina con END");
                    }
                    if (lineas[i].trim().isEmpty()) 
                          System.out.print(EnsambladorHC12.writeError(i+1, "\n\tERROR: linea Vacía"));
                    ...
                    else 
                    {
                        if(ensamblador.hasETIQUETA(lineas[i]))
                        {
                            ...
                            if(ensamblador.getEtiqueta().contains("\tERROR: "))
                                System.out.print("\n"+EnsambladorHC12.writeError(i+1, ensamblador.getEtiqueta()));
                            else
                            ...
                            {
                                System.out.print(EnsambladorHC12.writeError(i+1, "CODOP = null\n\tERROR: Si existe una etiqueta debe existir otro token más"));
    ...
    public String analizarLinea(int LINE_NUMBER, String[] palabras) {
        ...
        if(this.getCodop().contains("\tERROR: "))
            z.append(EnsambladorHC12.writeError(LINE_NUMBER, this.getCodop()));
        else
   ...
   public String validarETIQUETA(String palabra ) {
    ...
    public static String writeError(int LINE_NUMBER, String FILE_CONTENT){
        ...
        catch (FileNotFoundException ex) {Logger.getLogger(EnsambladorHC12.class.getName()).log(Level.SEVERE, null, ex);}
        return null;
    }
}        
